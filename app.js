	...
	switch (e.target.id) {
		case "swatch1": {
			root.style.setProperty(
				"--column-bg",
				"linear-gradient(#11a60d, rgba(13, 160, 165, 0.05))"
			);
			break;
		}
		case "swatch2": {
			root.style.setProperty(
				"--column-bg",
				"linear-gradient(#a6520d, rgba(166, 82, 13, 0.05))"
			);
			break;
		}
		case "swatch3": {
			root.style.setProperty(
				"--column-bg",
				"linear-gradient(#3b0da6, rgba(59, 13, 166, 0.05))"
			);
			break;
		}
		case "swatch4": {
			root.style.setProperty(
				"--column-bg",
				"linear-gradient(#a60d8e, rgba(166, 13, 143, 0.05))"
			);
			break;
		}
		case "swatch5": {
			root.style.setProperty(
				"--column-bg",
				"linear-gradient(#0da2a6, rgba(13, 160, 165, 0.05))"
			);
			break;
		}
		default:
...
function changeBoardType(e) {
	if (e.target.id == "board-grid-icon") {
		[...GAME.DOMColumns].forEach(col =>
			col.classList.remove("column-visible")
		);
